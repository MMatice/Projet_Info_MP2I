...
let _ =
    ...
    let hballe = ref 250 in (* Hauteur de départ de la balle et gerera la hauteur de la balle
							   dans le temps*)
    ...
    (*ça nous donne donc une line noir qui à l'allure d'une courbe  *)
        ...
        (current_x() + a, Random.int 100 + 150) 
    ...
    while true do
		...
		if (action.keypressed && screen.(25).(!hballe - 5) <> Sable)
		then begin
			touche_sable := false;								
			set_color sky_color;         (* Permet d'effacer *)
			fill_circle 25 !hballe 5;   (* l'ancienne balle *)
			set_color green;			 (* Dessine la nouvelle *)
			fill_circle 25 (!hballe - 1) 5;
			hballe := !hballe - 1;
			end;
    done